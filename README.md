...
CTD devices directly measure the conductivity, temperature, and pressure in seawater; however, the direct data can be used to calculate other values, such as depth and speed of sound. Depth is calculated using the equation depth = pressure*1000 / (density * gravitational acceleration) [SI units], and speed of sound is calculated using speed of sound = 1449.2 + 4.6T – 0.055T^2 +0.00029T^3 + (1.34-0.01T)*(S-35)+ 0.016Z [SI units] where T = temperature, S = salinity, and Z = depth. 
